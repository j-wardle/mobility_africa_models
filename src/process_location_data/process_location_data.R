...
## Code to tidy existing dataset with population weighted centroids (data from 2015)
## Update with 2006 estimates when we have them from Wes
## Some manual cleaning so do for each country in turn

## PORTUGAL

adm2_populations <- read.delim("data/adm2-fixed.txt", encoding = 'UTF-8') %>% 
  filter(ADM0 == "Portugal") %>% 
  clean_names()

id_names <- read.table("data/mobile_data/portugal/id_list.txt") %>% 
  rename(id = V1,
         name = V2)

## Some municipalities have two entries with different population sizes
## Some are on islands so we exclude, others have been labelled incorrectly (TAVIRA & GUIMARAES)

adm2_populations <- adm2_populations %>%
  mutate(adm2 = replace(adm2, adm2 == "Alcoutim" & pop == 25276, "Tavira"),
         adm2 = replace(adm2, adm2 == "Braga" & pop == 158613, "Guimaraes")) %>% 
  filter(!(adm2 == "Calheta")) %>%   # two municipalities on islands of Madeira and Azores
  filter(!(adm2 == "Lagoa" & pop == 9263)) # remove municipality on Azores


## Clean adm2 names to match municipalities in id_names, then filter to keep only matches

adm2_populations$adm2 <- iconv(adm2_populations$adm2, from = "UTF-8", to = "ASCII//TRANSLIT")
adm2_populations$adm2 <- gsub(" ", "_", toupper(adm2_populations$adm2))

portugal_location_data <- adm2_populations %>% 
  filter(adm2 %in% id_names$name) %>% 
  select(adm2, pop, weighted_centroid_lon, weighted_centroid_lat) %>% 
  rename(location = adm2,
         population = pop,
         x = weighted_centroid_lon,
         y = weighted_centroid_lat) %>% 
  arrange(location)

# This file does not have the outlier population sizes that were in the data Moritz shared
saveRDS(portugal_location_data, "data/portugal_location_data.rds")
