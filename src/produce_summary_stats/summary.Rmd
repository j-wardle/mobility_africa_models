...
not_adm0 <- est[est$data_spatial_resolution != "adm0", ]
est <- est[est$bibkey != "huang2013open", ]
## Exclude huang as they are based on non-African data
x <- length(unique(est$country))
y <- length(unique(africa[! africa %in% not_adm0$country]))
```
...
est_yes <- y$n[y$data_available == "yes"] + nrow(air_vol) ## include huang, which is open-acess
```
